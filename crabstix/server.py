...
from libtaxii.constants import (CB_STIX_XML_111, VID_TAXII_XML_11)


class CrabSTIXServer(SocketServer.ThreadingTCPServer):

    """
    Listens on a user supplied port and forwards the messages
    ...
    def __init__(self, RequestHandlerClass, ConfigPath):
        ...
        """
        Overrides the SocketServer __init__ to add out conf
        and parser modules, doing this allows them to be
        called by the handler.

        To stop the config reader needing to be definined
        first, this module loads the config reader in the
        __init__ method and passes the host and port
        while initialising.

        :param server_address: (HOST,PORT)
        ...
        for module in os.listdir("./crabstix/parsers"):
            ...
            if module.endswith(".py") and module != "__init__.py":
                ...
                # Import the module, load the Parser class,
                # store in a dict by parser name
                self._parsers[module[:-3]] = (
                    getattr(
                        getattr(
                            crabstix.parsers,
                            module[:-3]),
        ...
        # Log Start
        self._logging.info("Server started on %s:%s" % (
                self._config["SERVER"]["host"],
                int(self._config["SERVER"]["port"])),
            "server.py",
            "informational")
...
class LogHandler(SocketServer.BaseRequestHandler):
    ...
    """
    Accepts TCP connections from Syslog, reads 1k chunks
    and passes them to the parser.

    On a successful parse, the STIX XML is passed of to
    the TAXII api specified in crabstix.conf
    """
    ...
    def make_client(self):
        ...
        if self.server._config["TAXII"]["authentication"] == "NONE":
            ...
            client.set_auth_credentials(
                {'username': self.server._config["TAXII"]["username"],
                 'password': self.server._config["TAXII"]["password"]})
        else:
            self.server._logging.error(
                "Failed to send message to TAXII endpoint: Invalid HTTP AUTH",
                "server.py",
                "taxii_error")
            return False
    ...
    def handle(self):
        ...
        while True:
            ...
            for parser in self.server._parsers:
                ...
                if not self.server._parsers[parser].identify(log_line):
                    continue
                ...
                if stix_log:
                    ...
                    if not self.server._config["TAXII"]["enabled"]:
                        continue
                    ...
                    cb = tm11.ContentBlock(
                        tm11.ContentBinding(CB_STIX_XML_111), stix_log)
                    ...
                    self.server._logging.error(
                        "%s parser faileure, log added to the unparsable log." % (parser), # noqa E501
                        "server.py",
                        "parse_error")
            ...
            if not identified_flag:
                self.server._logging.error(
                    "No matching parsers were found for a log, it will be added to the unparsable log.", # noqa E501
                    "server.py",
                    "parse_error")
